<?php
	...
	$sql="SELECT a.*,TO_CHAR(a.TANGGALLAPOR,'dd/mm/yyyy HH24:Mi:ss') AS TGL,TO_CHAR(a.TGLEKSEKUSI,'dd/mm/yyyy HH24:Mi:ss') AS TGLEKSE,CASE
               WHEN laststatus = 'Nyala' THEN 2
               WHEN laststatus = 'Nyala Sementara' THEN 1
               ELSE 0
            END
               AS kodestatus,
		       CASE 
  ...
  FROM MON_MOBILE_NEW2 a where a.id_tiket='".$idtiket."'and status not in ('Nyala (Catat Data Lap.)','Penugasan Regu') order by id_tiket desc";
	 ...
	 // echo $sql;
            ...
            while ($row = oci_fetch_array($sqlq)) {
			 ...
			 $data['KSTATUS'] = floatval($row['KODESTATUS']);
